import React, { useEffect } from 'react'
import '../assets/css/styles.css'

        export default function Plans() {
          useEffect(() => {
    import('../assets/js/scripts.js');

          }, []);
          return <div dangerouslySetInnerHTML={ { __html: `<!-- HEADER -->
  <header>
    <div class="container header-inner">
      <h1 class="logo neon-title">Streaming Lab</h1>
      <nav>
        <ul class="nav-list">
          <li><a href="index.html">Inicio</a></li>
          <li><a href="account.html">Cuentas</a></li>
          <li><a href="login.html">Iniciar sesión</a></li>
        </ul>
      </nav>
    </div>
  </header>

  <!-- PLANS -->
  <main class="container">
    <h2>Planes disponibles</h2>
    <div class="plans-grid">

      <!-- Plan Individual -->
      <section class="card plan">
        <img src="img/Captura de pantalla 2025-09-11 225509.png" alt="Plan Individual">
        <h3>Individual</h3>
        <p>1 cuenta vinculada · Acceso completo</p>
        <p class="price">$5.990 / mes</p>
        <button class="btn">Seleccionar</button>
      </section>

      <!-- Plan Familiar -->
      <section class="card plan">
        <img src="img/Captura de pantalla 2025-09-11 225444.png" alt="Plan Familiar">
        <h3>Familiar</h3>
        <p>Hasta 6 cuentas vinculadas · Administradores múltiples</p>
        <p class="price">$9.990 / mes</p>
        <button class="btn">Seleccionar</button>
      </section>

      <!-- Plan Corporativo -->
      <section class="card plan">
        <img src="img/Captura de pantalla 2025-09-11 225529.png" alt="Plan Corporativo">
        <h3>Corporativo</h3>
        <p>Hasta 20 cuentas · Estadísticas avanzadas y soporte dedicado</p>
        <p class="price">$14.990 / mes</p>
        <button class="btn">Seleccionar</button>
      </section>
    </div>

    <!-- Video -->
    <section class="video-embed card">
      <h3>¿Cómo funciona?</h3>
      <div class="video-wrapper">
        <iframe title="Demo" src="https://www.youtube.com/embed/dQw4w9WgXcQ" allowfullscreen></iframe>
      </div>
    </section>
  </main>

  <!-- FOOTER -->
  <footer>
    <p>&copy; 2025 Streaming Lab</p>
  </footer>

  <script src="js/scripts.js"></script>` } } />
        }
